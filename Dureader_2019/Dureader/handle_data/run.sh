...
paragraph_extraction ()
{
    ...
    cat $SOURCE_DIR/testset/search.test1.json | python paragraph_extraction.py test \
            > $TARGET_DIR/testset/search.test1.json
    cat $SOURCE_DIR/testset/zhidao.test1.json | python paragraph_extraction.py test \
            > $TARGET_DIR/testset/zhidao.test1.json
    ...
    cat $SOURCE_DIR/devset/search.dev.json | python paragraph_extraction.py test \
