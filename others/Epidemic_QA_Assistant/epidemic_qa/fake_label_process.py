...
class Process(object):
    ...
    def make_test_data(self, topk_path, pkl_name):
        ...
        with open('./data/similarity/bm25_test_2gram_top30.dict', 'rb') as f:
            test_topk = pickle.load(f)

        topk.update(test_topk)

        print('4500_1643:', len(topk))
        ...
        for qid, question in zip(test_data['id'], test_data['question']):
            test_question[qid] = question

        test_data = pd.read_csv('vote.csv', sep='\t')
        for id, docid, answer in zip(test_data['id'], test_data['docid'], test_data['answer']):
            sample = {}
            sample['top'] = topk[id][:15]
            sample['text'] = 'none'
            sample['answer'] = answer
            sample['docid'] = -1
            sample['question'] =  test_question[id]
            data.append(sample)
